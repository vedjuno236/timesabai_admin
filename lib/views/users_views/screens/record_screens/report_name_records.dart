...
class ReportNameRecord extends StatefulWidget {
  ...
  const ReportNameRecord({super.key, required this.searchName,required this.monthDate});
...
class _ReportEmployeeState extends State<ReportNameRecord> {
  ...
  Future<Uint8List> generateDocument(PdfPageFormat format) async {
    ...
    for (var employeeDoc in employeeSnapshot.docs) {
      ...
      final employeeName = employeeDataMap['name']?.toString().toLowerCase() ?? '';
      if (widget.searchName.isNotEmpty && !employeeName.contains(widget.searchName.toLowerCase())) {
